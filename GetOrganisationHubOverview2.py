    ...
    try:
        ...
        if organisation_details and (organisation_details['permissionid'] <= 2):
            ...
            devices_details_sql = f"""
                ...
                SELECT * FROM lighting_devices UNION SELECT * FROM controller_devices
            """
            cursor.execute(devices_details_sql,
                           ([org_uuid] + [hub_uuid] + driver_devices + [org_uuid] + controller_devices))
...
def get_hub_details(cursor, org_uuid, organisation_details, hub_uuid):
    try:
        ...
        hub_details_sql = f"""
            ...
            ON a.hubUUID = %s AND a.organisationUUID = %s
        """
